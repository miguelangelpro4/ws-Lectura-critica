...
import com.SEII.repositories.ProfesoresRepository;
...
public class ProfesoresService {
    ...
    public Profesores insert(Profesores p) {
    ...
    public Profesores findById(int id) {
        Optional<Profesores> result = repository.findById(id);
        if (result.isPresent()) {
            return result.get();
        } else {
            return null;
    ...
    public boolean update(Profesores p) {
        try {
            repository.save(p);
            return true;
        } catch (Exception e) {
